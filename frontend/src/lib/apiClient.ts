...
import { handleDemoApiRequest, isDemoMode } from "@/demo";
 ...
 * Using axios for HTTP requests (or in-memory mocks in demo mode).
 */
async function demoOrAxios<T>(
	method: string,
	endpoint: string,
	...
	if (isDemoMode) {
		return handleDemoApiRequest<T>(method, endpoint, body);
	}
	return request();
}
...
export const apiClient = {
	get: async <T>(
		...
		return demoOrAxios("GET", endpoint, () =>
			axiosInstance
	...
	delete: async <T>(
		...
		return demoOrAxios("DELETE", endpoint, () =>
			axiosInstance
