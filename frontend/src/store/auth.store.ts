	...
	persist(
		(set, get) => ({
			...
			resolveCognitoGroups: async (): Promise<string[]> => {
				if (isDemoMode) {
					const groups = getDemoPersona().groups;
			...
			checkAuth: async (): Promise<void> => {
				if (isDemoMode) {
					...
					const hasSession =
						...
						sessionStorage.getItem("launchpad-demo-persona") != null;
					if (hasSession) {
						set({
							user: {
								userId: persona.cognitoSub,
			...
			login: async (
				...
				if (isDemoMode) {
					const persona = resolveDemoPersonaByEmail(credentials.email);
					set({ isLoading: true, error: null, email: credentials.email });
					if (!persona || credentials.password !== persona.password) {
						set({
							isLoading: false,
					...
					capturePosthogLoginSuccess("demo");
					return "success";
				}
